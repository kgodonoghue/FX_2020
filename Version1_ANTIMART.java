...
public class Version1_ANTIMART implements IStrategy {
    ...
    @Configurable("Stop Loss")      public int stopLoss = 20;
    @Configurable("Multiplier")     public double m = 2;
    @Configurable("MultiplierSL")   public double SLmult = 1;
    @Configurable("martSteps")      public int martSteps =2;
    @Configurable("martLimit")      public int martLimit =6;
   ...
   //instrument and current time is between selected range 
    ...
    private void recreateOrderBuy(Instrument instrument, ITick tick, double currentPriceBuy) throws JFException 
    {
            ...
            else if ((currentPriceBuy < stopLossPriceBuy) || (martCountBuy > martLimit))
            {
                    trackBuy = 1;
    ...
    private void recreateOrderSell(Instrument instrument, ITick tick, double currentPriceSell) throws JFException 
    {
            ...
            else if ((currentPriceSell > stopLossPriceSell) || (martCountSell > martLimit))
            {
                    trackSell =1;
    ...
    private void createOrdersBuy(ITick tick, Instrument instrument, double orderAmountBuy, OrderCommand orderCommand) throws JFException 
    {
        ...
        IOrder OrderBuy = engine.submitOrder(labelBuy, instrument, orderCommand, orderAmountBuy, tick.getAsk(),20);
        IMessage message = OrderBuy.waitForUpdate(20000, IOrder.State.FILLED);
        //resubmit order on rejection
        print("Order opened: " + message.getOrder());
        print("Order opened: " + message.getType());
        print("Order state: " + OrderBuy.getState());
        
        for (int i = 0; i < 2; i++) 
        {

                if (OrderBuy.getState() != State.FILLED && OrderBuy.getState() != State.OPENED) 
                {
                    print("test");
                    OrderBuy = engine.submitOrder(labelBuy, instrument, orderCommand, orderAmountBuy, tick.getAsk(),20);
                    OrderBuy.waitForUpdate(200000);
                    print("Order opened: " + message.getOrder());
                    print("Order opened: " + message.getType());
                    print("Order state: " + OrderBuy.getState());
                }
        ...
        IMessage message = OrderSell.waitForUpdate(20000, IOrder.State.CANCELED);
        //resubmit order on rejection
        print("Order opened: " + message.getOrder());
        print("Order opened: " + message.getType());
        print("Order state: " + OrderSell.getState());        
       
        for (int i = 0; i < 2; i++) 
        {

                if (OrderSell.getState() != State.FILLED && OrderSell.getState() != State.OPENED) 
                {
                    print("test");
                    OrderSell = engine.submitOrder(labelSell, instrument, orderCommand, orderAmountSell, tick.getBid(),20);
                    OrderSell.waitForUpdate(2000);
                    print("Order opened: " + message.getOrder());
                    print("Order opened: " + message.getType());
                    print("Order state: " + OrderSell.getState());
                }
